...
async def fetch_one(
    ...
    async with sem:
        try:
            ...
            out_path = OUTPUT_DIR / f"{group_name}_{name}.png"
            ...
            desc = f"{name}"
            result[f"{group_name}_{name}"] = {
                "desc": desc,
                "url": f"/assets/heybox/{group_name}_{name}.webp",
            }
...
async def main() -> None:
    ...
    with INPUT_JSON.open(encoding="utf8") as f:
        data: list[Dict[str, Any]] = json.load(f)["data"]
    ...
    async with httpx.AsyncClient(limits=limits, timeout=timeout, http2=True) as client:
        tasks = []
        for group in data:
            tasks.extend(fetch_one(client, sem, group["group_name"],item, result) for item in group["emojis"])
